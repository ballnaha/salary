...
(function(moment) {
  ...
  DaysSetConverter.prototype.calculate = function(daysToAdd) {
    ...
    if (daysLeft>=0){
        ...
        while (daysLeft > 0) {
            ...
            var included = str_inclusions.length != 0 && str_inclusions.indexOf(resultDate.format("MM/DD/YYYY"))>=0;
            if (included || ((this.weekdays.indexOf(resultDate[weekdayFunc]()) >= 0) && (str_exclusions.length == 0 || str_exclusions.indexOf(resultDate.format("MM/DD/YYYY")) < 0))) {
                daysLeft--;
            }
        }
    } else {
  ...
  var parseSet = function(set) {
    ...
    if (set) {
      ...
      for (;i<l;i++) {
        str_exclusions.push(moment(set[i]).format("MM/DD/YYYY"));
      }
    }
